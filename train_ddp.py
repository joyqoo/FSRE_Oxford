...
from datasets.robotcar_dataset import RobotcarDataset
...
class Trainer:
    def __init__(self, options):
        ...
        print("訓練模型名稱:\n  ", self.opt.model_name)
        print("模型和 Tensorboard events files 會儲存在:\n  ", self.opt.log_dir)
    ...
    def compute_depth_losses(self, inputs, outputs, losses):
        """計算相關的深度指標，可以在訓練期間進行監控
        並非完全精準，因為它是整個batch 的平均值，僅用於參考性能
         """
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(3, self.opt.batch_size)): # 最多寫入四張圖片
    ...
    def load_model(self):
        ...
        assert os.path.isdir(self.opt.load_weights_folder), \
            "無法找到此資料夾 {}".format(self.opt.load_weights_folder)
        print("從 {}".format(self.opt.load_weights_folder),'載入模型')

        for n in self.trainer.models:
            print("讀取 {} 權重...".format(n))
        ...
        if os.path.isfile(optimizer_load_path):
            ...
            print("找不到 Adam 權重檔 ，所以 Adam 採用隨機初始化")
